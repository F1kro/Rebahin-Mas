...
import { cn } from '../lib/utils'; // Sesuaikan path-nya dengan struktur folder kamu
            ...
            {seasonNumbers.map((season) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "px-6 py-2 border-[3px] border-black font-black text-sm uppercase italic transition-all active:translate-y-1 active:shadow-none shadow-[4px_4px_0_0_rgba(0,0,0,1)]",
                    // LOGIC WARNA:
                    isActive
                      ? "bg-[#FF0000] text-white shadow-none translate-y-1 border-black" // Saat Aktif: Tetap Merah di semua mode
                      : "bg-white text-black hover:bg-yellow-300 dark:bg-slate-800 dark:text-white dark:border-white" // Saat Tidak Aktif: Adaptif
                  )}
                >
                  Season {season}
                </button>
          ...
          {currentSeasonEpisodes.map((episode, episodeIndex) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "group relative border-[3px] border-black p-4 text-left transition-all hover:-translate-y-1 hover:-translate-x-1 hover:shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] active:translate-x-0 active:translate-y-0 active:shadow-none",
                  "bg-white text-black", // Light Mode
                  "dark:bg-slate-900 dark:text-white dark:border-slate-600" // Dark Mode
                )}
              >
                ...
                <div className="flex items-center space-x-4 mt-2">
                  ...
                  <div className="flex-1 overflow-hidden">
                    <h4 className="font-black text-sm uppercase italic leading-tight line-clamp-2 transition-colors group-hover:text-[#FF0000] dark:text-white">
                      {pickEpisodeLabel(episode, episodeIndex)}
                    </h4>
                  </div>
                </div>
  
                {/* Garis Dekorasi Neobrutalism di pojok */}
                <div className="absolute bottom-2 right-2 opacity-20 group-hover:opacity-100 transition-opacity">
                  <div className="w-4 h-[2px] bg-current mb-1 skew-x-12"></div>
                  <div className="w-6 h-[2px] bg-current skew-x-12"></div>
                </div>
              </button>
            );
          })}
